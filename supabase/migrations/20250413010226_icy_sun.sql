/*
  ...
  1. New Tables
    - `waf_templates`
      ...
      - `expression` (text)
      - `action` (text)
      - `action_parameters` (jsonb, nullable)
      - `is_core` (boolean)
      - `created_at` (timestamp)
      - `updated_at` (timestamp)

  2. Security
    - Enable RLS on `waf_templates` table
    - Add policy for admin to manage all templates
    - Add policy for other users to read templates
*/
...
-- Policy for admin (steve@shadowtek.com.au) to manage all templates
...
-- Policy for other users to read templates
